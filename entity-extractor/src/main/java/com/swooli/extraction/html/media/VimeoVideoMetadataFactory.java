...
import com.swooli.extraction.html.media.VimeoVideoMetadataFactory.VideoStrategy;
...
public class VimeoVideoMetadataFactory implements VideoMetadataFactory<VideoMetadata> {

    public static final String VIDEO_METADATA_PATTERN = "http%s://vimeo.com/api/v2/video/%s.json";

    public static final String BASE_VIDEO_URL_PATTERN = "http%s://player.vimeo.com/video/%s";

    public static final int DEFAULT_CACHE_SIZE = 1000;

    private static final Logger logger = LoggerFactory.getLogger(VimeoVideoMetadataFactory.class);
    ...
    private String thumbnailFieldName = "thumbnail_medium";
    ...
    public static enum VideoStrategy {
        ...
        IFRAME {
            ...
            public String extractVideoId(final String url) {
                ...
                // example: http://player.vimeo.com/video/id

                final UrlParser parser = new UrlParser();
                final UrlInfo urlInfo = parser.parseUrl(url);
                if("player.vimeo.com".equals(urlInfo.getHost())) {
        ...
        OBJ_EMBED {
            ...
            public String extractVideoId(final String url) {
                ...
                // example: http://vimeo.com/moogaloop.swf?clip_id=id

                final UrlParser parser = new UrlParser();
                final UrlInfo urlInfo = parser.parseUrl(url);
                if("vimeo.com".equals(urlInfo.getHost())) {
                    if(urlInfo.getPath() != null && urlInfo.getPath().equals("/moogaloop.swf")) {
                        final StringMultivaluedMap params = parser.parseQuery(urlInfo.getQuery());
                        final String id = params.getFirst("clip_id");
                        if(id.contains("/")) {
                            return null;
                        } else {
                            return id;
        ...
        DEFAULT {
            ...
            public String extractVideoId(final String url) {
                ...
                // example: http://vimeo.com/id

                final UrlParser parser = new UrlParser();
                final UrlInfo urlInfo = parser.parseUrl(url);
                if("vimeo.com".equals(urlInfo.getHost())) {
                    String id = urlInfo.getPath();
    ...
    public VimeoVideoMetadataFactory() {
        this(DEFAULT_CACHE_SIZE);
    }

    public VimeoVideoMetadataFactory(final int cacheSize) {
        ...
        videoUriParams.put("title", Arrays.asList("0"));
        videoUriParams.put("&amp;byline", Arrays.asList("0"));
        videoUriParams.put("&amp;portrait", Arrays.asList("0"));
    }
    ...
    private VideoMetadata getVideoMetadata(final String videoId) {
        ...
        final ResponseHandler<VideoMetadata> handler = new ResponseHandler<VideoMetadata>() {
            ...
            public VideoMetadata handleResponse(final HttpResponse response) throws IOException {
                ...
                result = mapper.readValue(entity.getContent(), new com.fasterxml.jackson.core.type.TypeReference<List<Map<String, Object>>>() {});

                if(result.isEmpty()) {
                    return null;
                }

                final String thumbnailUrlString = (String) result.get(0).get(thumbnailFieldName);
                ...
                // title
                final String title = (String) result.get(0).get(titleFieldName);
                if(title == null) {
        ...
        try {
            ...
            logger.warn("Failed creating video metadata for Vimeo URL {} due to {}", new Object[] {url, ioe, ioe});
            addToCache(videoId, new FailedVideoMetadata(url));
            return null;
        } finally {
